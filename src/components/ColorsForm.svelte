...
<div class="flex flex-col space-y-2">
    ...
    <button on:click={addColor}
            class="border-b-2 border-teal-700 bg-glass py-3 text-sm font-bold uppercase text-slate-50 transition-all duration-200 ease-in-out hover:bg-teal-700 hover:text-white">
        Add new
    </button>
</div>
            ...
            {#each $colorStore as color, index}
                <div class="flex flex-col space-y-4">
                    <div>
                        ...
                        <span class="text-xs text-gray-400 -mt-1.5">
                            Color name changes apply immediately
                        </span>
                    </div>
                    <div class="relative">
                        ...
                        <span class="text-xs text-gray-400 -mt-1.5">Without #</span>
                        <button type="button" on:click={() => updateColor(color.id, color.name, color.hex)}
                                class="size-9 absolute right-0 top-0 bg-white hover:bg-gray-200 transition-all duration-150 ease-in-out rounded-md flex items-center justify-center">
